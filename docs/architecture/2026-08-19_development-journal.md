...
## 阶段 2.20：工程维护——提交积压工作、README 同步与死代码清理（2026-08-24）

- 提交 8 月 19 日以来积压的未提交工作树：React 全模块迁移、渲染器安全加固、核心模块操作状态加固、Agent 操作契约与验证脚本（提交 `ab78462`，41 个文件，+1683/−185）。
- 更新 `README.md` 至 v0.7.0 当前状态：React 迁移完成情况、安全边界、验证命令与已知限制（原 README 仍停留在"下一步替换存储能力"的旧阶段描述）。
- 清理 `electron/main.cjs` 冗余分支：`if (app.isPackaged) … else …` 两个分支完全相同，收敛为单一 `win.loadURL(entryUrl)`。
- 同步 `docs/INDEX.md` 安装包版本行为 0.7.0。
- 验证：`node --check electron/main.cjs` 通过；`verify:ipc` 57 路渲染器通道契约通过；`verify:security` 12 项安全边界通过。变更仅涉及加载路径收敛与文档，不改变运行行为，未触发打包级回归。
